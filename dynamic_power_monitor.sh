#!/usr/bin/env bash
# Dynamic-Power Live Monitor  v3.2.1
# – smooth updates
# – instant full refresh on key-press OR when profile changes in dynamic mode
set -u
...
REFRESH=1           # force full clear on first loop
LAST_PROF=""

while true; do
  (( REFRESH )) && { clear; REFRESH=0; }

  # -------- collect live data --------
  AC=$([[ -f $AC_PATH && $(<"$AC_PATH") -eq 1 ]] && echo "Plugged-In" || echo "Battery")
  LOAD=$(awk '{print $1}' /proc/loadavg)
  PROF=$(get_prof)
  [[ $PROF != "$LAST_PROF" && $(<"$OVERRIDE_FILE") == dynamic ]] && REFRESH=1
  LAST_PROF=$PROF
